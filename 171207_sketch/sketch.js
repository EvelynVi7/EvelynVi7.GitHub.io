...
function preload() {
  table = loadTable("Memorials_in_DC.csv", "csv", "header");
...
function Monument(id) {
  ...
  this.radius = random([10, 20, 30]);
  this.col = random(["#d7c6cf", "#a2798f", "#8caba8"]);
  this.type = random(["statue", "building", "sculpture"]);
  this.topic = random(["culture", "political", "religious"])
  var fillCol = color(this.col)
  ...
  this.writeText = function() {
    if (dist(mouseX, mouseY, this.x, this.y) < this.radius) {
      ...
      text("Size: S", width - 800 + margin, 80);
      text("Material: Bronze", width - 800 + margin, 100);
